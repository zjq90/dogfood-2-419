    ...
    <main class="user-list-container" role="main">
        <div class="user-list-main">
            <header class="user-list-header">
                <h1 class="user-list-title">所有人的列表</h1>
            </header>
                    ...
                    <article class="user-list__item">
                        ...
                        <div class="user-list__actions">
                            <a href="/iDouBan/DoumailServlet?method=doumail_show&to_user_id=${u.userId}&nickname=${u.nickname}" 
                               target="_blank" class="user-list__action">
                                发豆邮
                            </a>
                            <a href="EveryoneListServlet?pre_method=attention&method=everyone_list&toUserId=${u.userId}&currentPage=${requestScope.p.currentPage}" 
            ...
            <c:if test="${requestScope.p.totalPage!=0}">
                <nav class="user-list-pagination" aria-label="分页导航">
                    ...
                    <div class="user-list-pagination__links">
                        <a href="EveryoneListServlet?method=${requestScope.method}&currentPage=1" class="user-list-pagination__link">首页</a>
                        <a href="EveryoneListServlet?method=${requestScope.method}&currentPage=${(requestScope.p.currentPage==1)?1:requestScope.p.currentPage-1}" class="user-list-pagination__link">上一页</a>
                        <a href="EveryoneListServlet?method=${requestScope.method}&currentPage=${(requestScope.p.currentPage==requestScope.p.totalPage)?requestScope.p.totalPage:requestScope.p.currentPage+1}" class="user-list-pagination__link">下一页</a>
                        <a href="EveryoneListServlet?method=${requestScope.method}&currentPage=${requestScope.p.totalPage}" class="user-list-pagination__link">尾页</a>
                    </div>
                </nav>
            </c:if>
        </div>
