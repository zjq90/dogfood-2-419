    ...
    <main class="page-container" role="main">
        <div class="page-content">
            <section class="page-content__main" aria-labelledby="page-title">
                <header>
                    <h1 id="page-title" class="page-title">黑名单</h1>
                </header>
                
                <c:choose>
                    <c:when test="${empty requestScope.p.objects}">
                        <div class="empty-state" role="status">
                            <p class="empty-state__text">您的黑名单是空的</p>
                        </div>
                    </c:when>
                    <c:otherwise>
                        <ul class="user-list" role="list" aria-label="黑名单列表">
                            <c:forEach items="${requestScope.p.objects}" var="u">
                                <li class="user-card">
                                    ...
                                    <div class="user-card__actions">
                                        <a href="${pageContext.request.contextPath}/EveryoneListServlet?pre_method=cancel_black&method=blacklist_list&toUserId=${u.userId}&currentPage=${requestScope.p.currentPage}" 
                        ...
                        <c:if test="${requestScope.p.totalPage != 0}">
                            <nav class="pagination" role="navigation" aria-label="分页导航">
                                ...
                                <a href="${pageContext.request.contextPath}/FriendListServlet?method=blacklist_list&currentPage=1" class="pagination__link">首页</a>
                                <a href="${pageContext.request.contextPath}/FriendListServlet?method=blacklist_list&currentPage=${(requestScope.p.currentPage == 1) ? 1 : requestScope.p.currentPage - 1}" 
                                ...
                                <a href="${pageContext.request.contextPath}/FriendListServlet?method=blacklist_list&currentPage=${(requestScope.p.currentPage == requestScope.p.totalPage) ? requestScope.p.totalPage : requestScope.p.currentPage + 1}" 
                                ...
                                <a href="${pageContext.request.contextPath}/FriendListServlet?method=blacklist_list&currentPage=${requestScope.p.totalPage}" class="pagination__link">尾页</a>
                            </nav>
                        </c:if>
                    </c:otherwise>
                </c:choose>
